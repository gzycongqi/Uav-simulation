...
class Sensor:
    def __init__(self, image_path, scale_factor=0.5, sensor_data=None):
        ...
        self.image_rect = self.image.get_rect()
        self.data = sensor_data if sensor_data is not None else 0  # 数据属性，默认为0

    def send_data(self):
        return self.data  # 返回传感器的数据
...
class DroneSimulation:
    def __init__(self, screen_width=2000, screen_height=1000, step=0.5):
        ...
        self.sensors = [
            Sensor('sensor.png', scale_factor=0.5, sensor_data=10),  # 给传感器初始数据
            Sensor('sensor.png', scale_factor=0.5, sensor_data=20),
            Sensor('sensor.png', scale_factor=0.5, sensor_data=30)
        ]
    ...
    def draw(self):
        ...
        for i, sensor in enumerate(self.sensors):
            ...
            self.drone.receive_data(data)  # 将传感器的数据传给无人机进行计算
        ...
        info_texts = [
            ...
            f"Sensor 1 Data: {self.sensors[0].data}",
            f"Sensor 2 Data: {self.sensors[1].data}",
            f"Sensor 3 Data: {self.sensors[2].data}"
        ]
