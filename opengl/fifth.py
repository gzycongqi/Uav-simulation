...
def display():
    ...
    # 绘制带纹理的矩形平面代替原本的球体（使用图片作为纹理）
    if ground_texture is not None:
        glColor3f(1.0, 1.0, 1.0)  # 使得颜色不影响纹理
        glPushMatrix()
        glTranslatef(drone_pos[0], drone_pos[1], drone_pos[2])  # 移动到无人机的位置
        glBindTexture(GL_TEXTURE_2D, ground_texture)
        
        # 绘制矩形平面
        glBegin(GL_QUADS)
        glTexCoord2f(0.0, 0.0)
        glVertex3f(-drone_radius, 0.0, -drone_radius)
